...
public class WbSyncMessage {
    ...
    public void init() throws URISyntaxException, MalformedURLException {
        ...
        builder.addParameter("_", String.valueOf(System.currentTimeMillis()));
        builder.addParameter("jsonp", "jQuery112406869571085748343_1561447840951");
        builder.addParameter("message", "[{\"channel\":\"/meta/connect\",\"connectionType\":\"callback-polling\",\"id\":\"89\",\"clientId\":\"" + jsonObject.getString("clientId") + "\"}]");
        ...
        wbExecutor.execute(new Runnable() {
            ...
            public void run() {
                while (true) {
                    try {
                        ResponseEntity<String> connectEntity = restTemplate.exchange(connecteUri, HttpMethod.GET, new HttpEntity<>(finalHttpHeaders), String.class);
